...
class KpiController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Kpi'])){
			...
			try {
				$countKpi = count($_POST['Kpi']['kpi']);
				$percent = 0;
				for($x = 0;$x < $countKpi; $x++) {
					$modelLoop	= new Kpi;
					$modelLoop->job_id=$_POST['Kpi']['job_id'];
					$modelLoop->kpi=$_POST['Kpi']['kpi'][$x];
					$modelLoop->weight=$_POST['Kpi']['weight'][$x];
					$modelLoop->note=$_POST['Kpi']['note'][$x];

					$percent += $modelLoop->weight;
				...
				if ($result === TRUE) {
					$transaction->commit();
					$url = Yii::app()->createUrl('kpi/admin');
					$this->redirect($url);
	...
	{
		$provider = Job::model()->findByPK($data->job_id);
