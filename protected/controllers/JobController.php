...
class JobController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Job']))
		{
			...
			if (@!empty($_FILES['Job']['name']['specification'])) {
				$model->specification = $_POST['Job']['specification'];
				if ($model->validate(array('specification'))) { $model->specification = CUploadedFile::getInstance($model, 'specification'); }
				else { $model->specification = ''; }

				$model->specification->saveAs($path . '/' . time() . '_' . str_replace(' ', '_', strtolower($model->specification)));
				$model->specification = time() . '_' . str_replace(' ', '_', strtolower($model->specification));
			}
			if($model->save())
